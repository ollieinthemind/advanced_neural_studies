...
%preallocate
V=-70*ones(length(shift),length(t));
...
for w=1:length(shift);
    for k=1:length(t);
        if k==1;
            ...
            ah(k)=.07*exp(-(V(w,k)+70+shift(w))/20);
            bh(k)=1/(exp(-(V(w,k)+40+shift(w))/10)+1);
            ...
            gNa(k)=m3(k)*h(k)*gNabar;
            gK(k)=n4(k)*gKbar;
            V(w,k)=V(w,k-1)+(dt/C)*(Iinj(w)-(gNa(k)+gK(k)+gL)*V(w,k-1)+gNa(k)*ENa+gK(k)*EK+gL*EL);
...
xlabel('Time (ms)')
ylabel('Voltage (mV)')
title('Shifting h gate Voltage Dependence')
legend('10 mV shift', '5 mV shift','-5 mV shift','-10 mV shift')