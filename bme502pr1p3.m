...
for w=1:length(Kb);
    for k=1:length(t);
        if k==1;
            ...
            gNa(k)=m3(k)*h(k)*gNabar;
            gK(k)=Kb(w)*n4(k)*gKbar;
            V(w,k)=V(w,k-1)+(dt/C)*(Iinj(k)-(gNa(k)+gK(k)+gL)*V(w,k-1)+gNa(k)*ENa+gK(k)*EK+gL*EL);
...
xlabel('Time (ms)')
ylabel('Voltage (mV)')
title('AP with Different K+ Channel Blockages')
